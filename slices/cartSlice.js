...
export const cartSlice = createSlice({
    ...
    reducers: {
        ...
        removeFromCart: (state, action) => {
            ...
            let itemIndex = state.items.findIndex(item =>item.id == action.payload.id);// find the index of the item that we want to remove
            if(itemIndex >= 0){
                newCart.splice(itemIndex,1); //remove the item from the array
            }else{
